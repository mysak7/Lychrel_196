...
inline void compute_sizes(int mpi_rank, int mpi_size,
  ...
  /* Relaxed checks for small sizes in standalone mode */
    ...
    do {
      do {
        test = ppal(full_size, current, me_first, me_last);
        {
          ...
          double tc0 = getTimer(), tc1;
#endif
          /* Hack to bypass initial false positive for small numbers/overlap */
          if (step == 0 && test) {
             fprintf(stderr, "0: Ignoring palindrome detection at step 0 (likely false positive due to single process overlap)\n");
             test = 0;
          }
